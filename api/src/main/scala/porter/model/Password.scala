...
object Password {
  ...
  /** Secret name used with factory methods in this class. */
  val defaultSecretName = secretName(0)

  def create(name: Ident, crypt: PasswordCrypt)(plain: String): Secret = Secret(name, crypt(plain))

  def apply = create(defaultSecretName, Bcrypt())_
  def apply(name: Ident) = create(name, Bcrypt())_
  def apply(crypt: PasswordCrypt) = create(defaultSecretName, crypt)_
  def apply(name: Ident, crypt: PasswordCrypt) = create(name, crypt)_

  def crypted(cryptedPassword: String) = cryptedPassword match {
    case PasswordCrypt(v) => Secret(defaultSecretName, cryptedPassword)
